...
public:
    ...
    Q_DECL_CONSTEXPR inline QDoubleVector3D(const QVector3D vector);
    ...
    QVector3D toQVector3D() const { return QVector3D(xp, yp, zp); }
...
Q_DECL_CONSTEXPR inline QDoubleVector3D::QDoubleVector3D(const QVector3D vector)
    : xp(vector.x())
    , yp(vector.y())
    , zp(vector.z()) {}
